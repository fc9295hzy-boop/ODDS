...
from storage import init_db, save_snapshot, get_history
...
FOOT_CONFIG = {
    ...
    "matches": [],                  # liste de tuples (team_a, team_b) a suivre
}
...
TENNIS_CONFIG = {
    ...
    "matches": [],                  # liste de tuples (player_a, player_b) a suivre
}
...
async def run_pipeline(cfg):
    """Boucle infinie pour un sport donne. Lancer une instance par sport."""
    while True:
        for team_a, team_b in cfg["matches"]:
            await process_match(cfg, team_a, team_b)
...
async def start_tracking():
    ...
    if not FOOT_CONFIG["matches"] and not TENNIS_CONFIG["matches"]:
        log.warning("Aucun match configure dans FOOT_CONFIG['matches'] ou TENNIS_CONFIG['matches']. "
                    "Ajoute des tuples (equipe_a, equipe_b) / (joueur_a, joueur_b) avant de lancer.")
